...
namespace Unity.FPS.Gameplay
{
    ...
    public class ProjectileStandard : ProjectileBase
    {
        ...
        private List<Collider> ignoredColiders;         //Hit 판정시 무시하는 충돌체 리스트

        //충돌 연출
        public GameObject impackVfxPrefab;              //타격 이펙트
        [SerializeField] public float impactVfxLifeTime = 5f;
        ...
        new void OnShoot()
        {
            ...
            //무시 충돌 리스트 생성 - projectile을 발사하는 자신의 모든 충돌체를 가져와서 등록
            ignoredColiders = new List<Collider>();
            Collider[] ownerColliders = projectileBase.Owner.GetComponentsInChildren<Collider>();
            ignoredColiders.AddRange(ownerColliders);
        ...
        private void Update()
        {
            ...
            //Sphere Cast
            Vector3 displacementSinceLastFrame = root.position - lastRootPosition;
        ...
        bool IsHitValid(RaycastHit hit)
        {
            ...
            //
            if (ignoredColiders != null && ignoredColiders.Contains(hit.collider))
            {
                return false;
            }
